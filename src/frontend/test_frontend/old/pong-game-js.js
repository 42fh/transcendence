...
const { AlertCircle, Activity } = lucide;
  ...
  useEffect(() => {
    if (gameStarted) {
      const mockWebSocketData = [
        ...
        { type: 'event', message: 'Player 2 connected', timestamp: Date.now() },
        { type: 'event', message: 'Player 3 connected', timestamp: Date.now() + 1000 },
        { type: 'game_state', scores: [0, 1, 0, 0], timestamp: Date.now() + 2000 },
        { type: 'error', error: 'High latency detected', details: 'Ping > 100ms', timestamp: Date.now() + 3000 },
        { type: 'event', message: 'Ball collision with paddle 1', timestamp: Date.now() + 4000 },
        { type: 'game_state', scores: [0, 1, 1, 0], timestamp: Date.now() + 5000 },
        { type: 'error', error: 'Frame skip detected', details: 'Dropped 2 frames', timestamp: Date.now() + 6000 },
        { type: 'event', message: 'Power-up spawned: Speed boost', timestamp: Date.now() + 7000 }
      ];

      let index = 0;
      const interval = setInterval(() => {
        if (index < mockWebSocketData.length) {
          const data = mockWebSocketData[index];
      ...
      {showSetup ? (
        ...
        <div className="container mx-auto p-4">
          <div className="flex gap-4">
            {/* Main game area and monitoring panels */}
            <div className="flex-grow flex flex-col gap-4">
              {/* Top row with game canvas */}
            ...
            {/* Right sidebar with setup info and error monitor */}
              ...
              <div className="bg-white rounded-lg shadow-md p-4">
                ...
                {setupData && (
                  <div className="space-y-2 text-sm">
                    <div>
                      <span className="font-medium">Game ID:</span> {setupData.gameId}
                    </div>
                    <div>
                      <span className="font-medium">Player ID:</span> {setupData.playerId}
                    </div>
                    <div>
                      <span className="font-medium">Game Type:</span> {setupData.gameType}
                    </div>
                    <div>
                      <span className="font-medium">Players:</span> {setupData.numPlayers}
                    </div>
                    <div>
                      <span className="font-medium">Sides:</span> {setupData.numSides}
                    </div>
                    <div>
                      <span className="font-medium">Balls:</span> {setupData.numBalls}
                    </div>
                    <div>
                      <span className="font-medium">Shape:</span> {setupData.shape}
                    </div>
                    <div>
                      <span className="font-medium">Score Mode:</span> {setupData.scoreMode}
                    </div>
                  </div>
                )}
              </div>
...
// Setup form component remains the same...
