...
export default class World {
    ...
    addOrthographicCamera() {
        ...
        if (this.gui.debug) {
            this.gui.gui.add(this.camera.position, 'y').min(-5).max(10).step(0.01).name('Camera y');
            this.gui.gui.add(this.camera.position, 'z').min(-5).max(10).step(0.01).name('Camera z');
            this.gui.gui.add(this.camera.position, 'x').min(-5).max(10).step(0.01).name('Camera x');
        }
    }
    ...
    addGame(newGame, isASCII = false) {
        ...
        if (!isASCII) {
            ...
            this.composer = new AsciiEffect(this.renderer, ' .:-+*=%@#', { invert: true });
