                ...
                "games": [
                    {
                        ...
                        "player1": (
                            list(schedule.game.players.values_list("display_name", flat=True))[0]
                            if schedule.game.players.exists()
                            else None
                        ),
                        "player2": (
                            list(schedule.game.players.values_list("display_name", flat=True))[1]
                            if schedule.game.players.count() > 1
                            else None
                        ),
                        "nextGameUuid": str(schedule.game.source_game1.id) if schedule.game.source_game1 else None,
                        "score": None,  # Add game score logic if needed
                        "winner": schedule.game.winner.display_name if schedule.game.winner else None,
                        "sourceGames": [
                            str(schedule.game.source_game1.id) if schedule.game.source_game1 else None,
                            str(schedule.game.source_game2.id) if schedule.game.source_game2 else None,
                        ],
                    }
                    for schedule in tournament.games.through.objects.filter(round_number=round_num)
                    .select_related("game")
                    .order_by("match_number")
                ],
            }
            for round_num in tournament.games.through.objects.values_list("round_number", flat=True)
            .distinct()
