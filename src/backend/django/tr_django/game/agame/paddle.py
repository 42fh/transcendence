...
async def update_paddle(self, player_index, position):
    ...
    try:
        ...
        try:
            current_data = await self.redis_conn.hget(self.paddles_key, str(player_index))
            current_position = msgpack.unpackb(current_data)["position"] if current_data else 0.5
        except:
