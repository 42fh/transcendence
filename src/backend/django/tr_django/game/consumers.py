...
class PongConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            player_count = await self.game_manager.redis_conn.scard(self.game_manager.players_key)
            
            # Initialize game if no players exist with provided settings
            if player_count == 0:
                settings = {
            ...
            try:
                state_data = await self.game_manager.redis_conn.get(self.game_manager.state_key)
                if state_data:
    ...
    async def update_paddle_position(self, direction):     
        move_amount = self.player_values['move_speed'] * self.player_values['move_speed_boost'] 
    ...
    async def power_up_event(self, event):
        ...
        if target_player == self.player_index:
            if power_up_type == 'speed_boost':
                self.player_values['move_speed'] = effect_data.get('speed_multiplier', 1.0)
            elif power_up_type == 'reverse_controls':
                self.player_values['reverse_controls'] = effect_data.get('active', False)
            elif power_up_type == 'resize_paddle':
                self.player_values['paddle_size'] = effect_data.get('size_multiplier', 1.0)
