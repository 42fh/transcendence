    ...
    async def game_logic(self, current_state):
        def get_polygon_vertices():
            ...
            for i in range(self.num_sides):
                ...
                vertices.append({"x": math.cos(angle), "y": math.sin(angle)})
            return vertices
        ...
        def calculate_distance_to_side(point, side_index, vertices):
            ...
            t = max(
                0,
                min(1, (point_x * segment_x + point_y * segment_y) / segment_length_sq),
            )
        ...
        def calculate_paddle_collision(
            side_index, collision_info, ball, paddles, dimensions
        ):
            ...
            projected_len = (
                ball_to_paddle_x * dx + ball_to_paddle_y * dy
            ) / side_length

            # Calculate distance from ball to paddle surface
            dist_to_paddle = abs(
                ball_to_paddle_x * normal_x + ball_to_paddle_y * normal_y
            )
            ...
            if is_within_paddle_length and is_within_paddle_thickness:
                ...
                print(
                    f"  - Velocity: vx={ball['velocity_x']:.4f}, vy={ball['velocity_y']:.4f}"
                )
                ...
                print(
                    f"  - Center Position: x={paddle_center_x:.4f}, y={paddle_center_y:.4f}"
                )
                print(
                    f"  - Length: {paddle_length:.4f} (half={half_paddle_length:.4f})"
                )
                ...
                print(
                    f"  - Is Edge Hit: {abs(projected_len) > half_paddle_length * 0.9}"
                )
        ...
        def find_nearest_collision(ball, paddles, vertices):
            ...
            for i in range(self.num_sides):
                ...
                if paddle and paddle["active"]:
                    ...
                    paddle_collision = calculate_paddle_collision(
                        i, collision_info, ball, paddles, current_state["dimensions"]
                    )
                    if (
                        paddle_collision
                        and paddle_collision["type"] == "paddle"
                        and paddle_collision["distance"] < min_distance
                    ):
                        min_distance = paddle_collision["distance"]
                        best_collision = paddle_collision
                    elif (
                        paddle_collision["type"] == "miss"
                        and collision_info["distance"] <= ball["size"]
                    ):
                    ...
                    if (
                        collision_info["distance"] <= ball["size"]
                        and collision_info["distance"] < min_distance
                    ):
                        min_distance = collision_info["distance"]
                        best_collision = {
                            "type": "wall",
            ...
            if collision["type"] in ["paddle", "wall"]:
                ...
                if penetration > 0:
                    if collision["type"] == "paddle":
                        ...
                        ball["x"] = paddle_surface_x + normal_x * (
                            ball["size"] + BUFFER
                        )
                        ball["y"] = paddle_surface_y + normal_y * (
                            ball["size"] + BUFFER
                        )
                    else:
                        # For wall collisions, use the original wall projection
                        ball["x"] = collision["projection"]["x"] + collision["normal"][
                            "x"
                        ] * (ball["size"] + BUFFER)
                        ball["y"] = collision["projection"]["y"] + collision["normal"][
                            "y"
                        ] * (ball["size"] + BUFFER)
        ...
        for ball_idx, ball in enumerate(new_state["balls"]):
            ...
            if collision:
                if collision["type"] in ["paddle", "wall"]:
                    ...
                    post_collision = find_nearest_collision(
                        ball, new_state["paddles"], vertices
                    )
