...
class TournamentNotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        async with await GameCoordinator.get_redis(TournamentManager.REDIS_URL) as redis_conn:
            if await redis_conn.exists(f"tournament_pending_start:{self.tournament_id}"):
                tournament = await sync_to_async(Tournament.objects.get)(pk=self.tournament_id)
                await sync_to_async(TournamentManager.start_matchmaking)(tournament)
                await redis_conn.delete(f"tournament_pending_start:{self.tournament_id}")
    ...
    async def player_left(self, event):
        ...
        if message["type"] == "force_disconnect" and self.player_id == message["player_id"]:    
    ...
    async def tournament_notification(self, event):
        print("fdg")        
        if "player_id" not in event["message"] or str(self.player_id) ==   event["message"]["player_id"]:
            print("almooooost")
